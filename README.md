...
- **大盘复盘** - 支持A股和美股双市场，根据配置自动判断（指数行情、板块涨跌、资金流向）
...
**智能双市场支持：**
- 仅配置 `CN_STOCK_LIST` → 仅分析A股
- 仅配置 `US_STOCK_LIST` → 仅分析美股  
- 两者都配置 → 同时分析A股和美股

```
🎯 2026-01-10 A股大盘复盘
（标题根据配置自动调整为 "A股大盘复盘" / "美股市场复盘" / "全球市场复盘"）

📊 主要指数
A股指数（配置CN_STOCK_LIST时显示）：
- 上证指数: 3250.12 (🟢+0.85%)
- 深证成指: 10521.36 (🟢+1.02%)
- 创业板指: 2156.78 (🟢+1.35%)

美股指数（配置US_STOCK_LIST时显示）：
- 标普500: 4850.25 (🟢+0.65%)
- 纳斯达克: 15250.48 (🟢+1.12%)
